  ...
  async searchProfiles(filters: SearchFilters, currentUserId: number) {
    ...
    if (filters.isPremiumUser) {
      ...
      if (filters.appearance && filters.appearance.length > 0) {
        conditions.push(Prisma.sql`(u.profile_details->'mn_basic_details_draft'->>'appearance' IN (${Prisma.join(filters.appearance)}) OR u.profile_details->'basicDetails'->>'appearance' IN (${Prisma.join(filters.appearance)}))`);
      }
      ...
      if (filters.marriageGoalPlan && filters.marriageGoalPlan.length > 0) {
        conditions.push(Prisma.sql`(u.profile_details->'mn_basic_details_draft'->>'marriageGoalPlan' IN (${Prisma.join(filters.marriageGoalPlan)}) OR u.profile_details->'basicDetails'->>'marriageGoalPlan' IN (${Prisma.join(filters.marriageGoalPlan)}))`);
      }
