...
export class DeductPointsUseCase {
  constructor(private referralRepository: IReferralRepository) {}

  async execute(userId: number, points: number, reason: string): Promise<ReferralTransactionEntity> {
    ...
    return await this.referralRepository.executePointsTransaction(userId, points, "DEDUCT", reason);
  }
}
